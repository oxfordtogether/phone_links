...
RSpec.describe "add roles to caller", type: :system do
  ...
  it "adds admin role" do
    ...
    visit "/a/people/#{caller.person.id}"
    click_on "More actions"
    click_on "Add Admin role"

    expect(page).to have_current_path("/a/people/#{caller.person.id}/admin/new")
  ...
  it "cancels attempt to add pod leader role" do
    ...
    expect(page).to have_current_path("/a/people/#{caller.person.id}/pod_leader/new")
  ...
  it "cancels attempt to add caller role" do
    ...
    expect(page).to have_current_path("/a/people/#{pod_leader.person.id}/caller/new")

    click_on "Cancel"

    expect(page).to have_current_path("/a/people/#{pod_leader.person.id}/events")
    pod_leader.reload
