...
RSpec.describe "search cache", type: :model do
  ...
  # describe "searching pwds" do
  #   let!(:pwd) { create(:person_with_dementia, person: person) }
  #   let!(:pwd2) { create(:person_with_dementia, person: person2) }
  #   let!(:pwd3) { create(:person_with_dementia, person: person4) }
  #   let!(:carer) { create(:carer, person: person2) }
  #   let!(:volunteer) { create(:volunteer, person: person2) }
  #   let!(:dementia_adviser) { create(:dementia_adviser, person: person3) }
  ...
  #   it "returns only matching pwds" do
  #     expect(SearchCache.get_pwds("blog").records).to match_array([pwd, pwd2])
  #   end
  # end
end
